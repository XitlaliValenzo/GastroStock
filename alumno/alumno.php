<?php
 //session_start();
  ...
  $id = $_SESSION['ID'];
  $sql = "SELECT * FROM usuarios INNER JOIN grupo ON grupo.id_grupo = usuarios.grupo
  WHERE id = '$id'";
...
<div class="sticky-top">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
      <a class="nav-link" target="_blank" href="manual_de_usuario.pdf"><i class="fa-solid fa-circle-info"></i> </a>
      </li>
    </ul>
...
<!--Buscar-->
  <?php
    ...
    $ruta1 = "administrador/php_action/" . htmlspecialchars($row_buscar['imagen']);
    $ruta2 = "encargado/php_action/" . htmlspecialchars($row_buscar['imagen']);
    ...
    ?>
                   <img src="<?php echo htmlspecialchars($rutaImagen); ?>" class="card-img-top" alt="utensilio" style="object-fit: contain; width: 100%; height: 100%;" data-toggle="modal" data-target="#exampleModal1_<?php echo $i_buscar ?>">
                   <!-- Modal -->
<div class="modal fade" id="exampleModal1_<?php echo $i_buscar ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="row no-gutters">
    <div class="col-md-4">
      ...
      <img src="<?php echo htmlspecialchars($rutaImagen); ?>" alt="articulo" style="object-fit: contain; width: 100%; height: 100%; ">
      </div>
    </div>
    <div class="col-md-8">
      <div class="card-body">
...
<!--Fin buscar-->
  ...
  $query = "SELECT id_articulo, nombre, cantidad, descripcion, imagen FROM articulos WHERE estatus ='activo' ";
  ...
  if ($result2->num_rows > 0) {
        while($row2 = $result2->fetch_assoc()) { 
          ...
          <div class="col mb-4">
            <div class="card shadow" style="border-radius: 20px;" >
               <div style="align-items: center; height: 30vh;display: flex;">
                  <?php // Definir posibles rutas
    $ruta1 = "administrador/php_action/" . htmlspecialchars($row2['imagen']);
    $ruta2 = "encargado/php_action/" . htmlspecialchars($row2['imagen']);
    ...
    if (file_exists($ruta1)) {
        ...
        $rutaImagen = $ruta2;
                   ...
                   <img src="<?php echo htmlspecialchars($rutaImagen); ?>" class="card-img-top" alt="utensilio" style="object-fit: contain; width: 100%; height: 100%;" data-toggle="modal" data-target="#exampleModal_<?php echo $i?>">
        ...
        <div class="card shadow rounded" style="width: 100%; margin-bottom: 0.6rem;margin-left: 0.9rem;margin-right: 0.9rem;">
          ...
          <div class="col-4 d-flex align-items-center justify-content-center">
            ...
            <div style="align-items: center; height: 20vh;display: flex; justify-content: center; padding: 0.5rem;">
                <?php
    // Definir posibles rutas
    $ruta1 = "administrador/php_action/" . htmlspecialchars($art[1]);
    $ruta2 = "encargado/php_action/" . htmlspecialchars($art[1]);
...
<a href="alumno/php_action/agregar_cantidad.php?cant=<?php echo $ie ?>" ><button type="button" class="btn btn-danger" id="mas_art_<?php echo $ie ?>">
    <i class="fa-solid fa-plus fa-xs"></i>
  </button></a>
  ...
  <!--<button type="button" class="btn btn-danger" id="menos_art_<?php echo $ie ?>">
    <i class="fa-solid fa-minus fa-xs"></i></button>-->

    <a href="alumno/php_action/quitar_cantidad.php?cant=<?php echo $ie ?>" ><button type="button" class="btn btn-danger" id="menos_art_<?php echo $ie ?>">
    <i class="fa-solid fa-minus fa-xs"></i></button></a>

</div>

         <a href="alumno/php_action/eliminar_de_solicitud.php?in=<?php echo $ie ?>" style="text-decoration: none;margin-top:1.5rem ;"><button type="button" class="btn btn-outline-danger btn-block"><i class="fa-solid fa-trash"></i> Eliminar</button></a>
    </div>
  </div>
</div>
</div>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    cantidadInput.addEventListener("input", () => {
        ...
        actualizar_cantidad(ie, cantidadInput.value);
      }
    });
    function actualizar_cantidad(ie, cantidad) {
      fetch('alumno/php_action/actualizar_cantidad.php', {
        method: 'POST',
        headers: {
                'Content-Type': 'application/json'
            },
         body: JSON.stringify({ ie: ie, cantidad: cantidad })
      }).then(response => {
        if (response.ok) {
      ...
      .then(data => {
        if (data.success) {
          ...
          window.location.href = `home.php?m=${data.messageCode}`;
        }
      })
      .catch(error => {
...
<div class="form-group">
      ...
      <select id="asignatura" class="form-control" name="asignatura">
        ...
        $sqlMateria = "SELECT * FROM materias WHERE activo = 1 AND cuatrimestre = '$cuatrimestre' ";
        $resultMateria = $con -> query($sqlMateria);
  ...
  <div class="form-group">
    ...
    <select id="profesor" class="form-control" name="profesor">
        <option selected>Selecciona al profesor encargado...</option> 
    </select>
</div>
...
document.getElementById('asignatura').addEventListener('change', function() {
    ...
    fetch(`alumno/getProfesores.php?asignatura=${asignaturaId}`)
        .then(response => response.json())
        .then(data => {
            const profesorSelect = document.getElementById('profesor');
            profesorSelect.innerHTML = '<option selected>Selecciona al profesor encargado...</option>';
